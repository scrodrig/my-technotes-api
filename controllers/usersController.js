...
// @route   POST /users
// @access  Private
const createNewUser = asyncHadler(async (req, res) => {
...
// @route   PATCH /users
// @access  Private
const updateUser = asyncHadler(async (req, res) => {
...
// @route   DELETE /users
// @access  Private
const deleteUser = asyncHadler(async (req, res) => {
  const { id } = req.body
  if (!id) {
  ...
  if (notes?.length) {
    return res.status(409).json({ message: 'User has notes, delete notes first' })
  }
